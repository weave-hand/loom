...
> Python, and Go but is learning Rust and buck2. Read the full file at the PR head
> AND its diff for this PR:
> `gh pr diff <PR> -- <path>` (the diff) and `Read <path>` (full context).
> Also skim repo `CLAUDE.md` for accurate rationale.
>
> Produce a set of **inline teaching comments**. For each, pick a specific line (or
> a small line range) **that appears on the RIGHT side of the diff hunks** (added or
> unchanged-context lines shown in `gh pr diff`) — GitHub rejects comments on lines
> outside the diff. For each block explain, in Joe's terms: (1) what it does, (2) the
> Rust/buck2 idiom and a Bazel/Python/Go analogy, (3) why this idiom/design over the
> alternatives, (4) any non-obvious consideration (ownership, lifetimes, errors,
> async, sqlx macros, buck2 target wiring). Be detailed but tight; one comment per
> coherent block.
>
...
- GitHub rejects the **whole** review if any comment targets a line outside the
  diff hunks. If the POST fails on a bad line, drop the offending comment(s) (the
