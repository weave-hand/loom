...
impl MvWatermarks for MemoryControlPlane {
    ...
    async fn advance_mv_watermark(
        ...
        // An empty advance writes nothing, so it is a no-op success on BOTH backends — postgres's
        // trait impl loops over `advances` and calls the guarded CAS per element, so an empty slice
        // never reaches the def-existence guard. Short-circuit here to keep the two backends identical
        // on that degenerate input (nothing is written, so a defless key is harmless either way).
