...
#[allow(
    ...
    reason = "seed_table needs cp, store, table, columns, schema, batch, and file_prefix to fully specify a seeded table"
)]
...
#[allow(
    dead_code,
    reason = "shared test helper; used by compact_e2e but not all includers"
)]
#[allow(
    clippy::too_many_arguments,
    reason = "seed_table_absolute needs cp, store, root_url, table, columns, schema, batch, and file_prefix"
)]
