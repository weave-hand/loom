...
use datafusion_io::infer::{InferError, arrow_logical_type, infer_columns};

#[test]
fn maps_supported_arrow_types_to_logical_strings() {
...
fn infer_columns_carries_name_and_nullability_in_order() {
    ...
    assert_eq!(cols[0].ty, "long");
    assert!(!cols[0].nullable);
    assert_eq!(cols[1].name, "name");
    assert_eq!(cols[1].ty, "string");
