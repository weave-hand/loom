//! `consolidate_table` takes the SAME per-table advisory lock `flush_table`/
...
//! overwrite-commit — see the design note on `consolidate_table` itself. A full
...
//! flush/GC), `consolidate_table` cannot proceed past the gate, and once that
...
async fn consolidate_table_defers_to_a_held_table_lock() {
    ...
    // Declare the table CDC (keyed on `id`) — enough shape for `consolidate_table`
    ...
    // Hold the SAME per-table advisory lock `consolidate_table` must take, from
    ...
    assert!(
        ...
        "consolidate_table must defer while a concurrent holder has the table's advisory lock"
    );

    // Release the held lock — consolidate_table must now proceed past the
    ...
    let outcome = tokio::time::timeout(Duration::from_secs(5), consolidate_task)
        .await
        .expect("consolidate_table should proceed once the table lock is released")
        .expect("consolidate_table task should not panic");
    drop(outcome);
}
