...
- [ ] **DuckLake object-store S3 routing** `{#fut-ducklake-s3-routing area:iceberg status:deferred from:road-iceberg-real-object-store pr:- spec:-}`
  [[road-iceberg-real-object-store]] is Iceberg-only — DuckLake's `service_runtime::local_store` continues to write `file://` Parquet. Routing DuckLake through the same `LOOM_WAREHOUSE_URI`/`AWS_*` config is an additive follow-up, gated on a real DuckLake-over-S3 consumer (the Iceberg-default direction makes this low-priority). Helm/deploy S3 wiring is tracked under [[fut-deploy-followups]].
