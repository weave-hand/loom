...
- [ ] **Backfill of per-column stats for pre-existing files** `{#fut-iceberg-stats-backfill area:iceberg status:deferred from:2026-06-21-iceberg-per-column-stats-design pr:- spec:-}`
  Per-column stats are recorded only for files written after [[road-iceberg-percolumn-stats]] landed; files written earlier have empty `column_stats` and are always kept by the pruner (safe, but never skipped). A bulk pass re-reading the Parquet footers of existing live data files to populate `data_file_column_stat` is deferred to a follow-up.
...
- [ ] **Iceberg ActionEngine impl** `{#fut-iceberg-actionengine area:iceberg status:deferred from:2026-06-15-actions-part1-design pr:- spec:-}`
