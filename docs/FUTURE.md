...
- [x] **Iceberg schema evolution** `{#fut-iceberg-schema-evolution area:iceberg status:promoted from:iceberg-roadmap pr:- spec:-}`
  Promoted to [[road-iceberg-schema-evolution]] (additive add-column + detect/reject slice): appended nullable columns evolve the mirror MVCC-correctly and populate the reserved `schema_version`; every non-additive change (drop/rename/retype/reorder/non-nullable add) is rejected instead of silently dropped. `field_id` persistence + rename/drop/type-promote and time-travel as-of-schema reconstruction remain deferred to a full-evolution slice.
