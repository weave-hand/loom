...
**Interfaces:**
- Produces: a `//third-party:argon2` buck target depending crates can list in `deps`. (`sha2`, `hex`, `getrandom`, `time`, `serde`, `serde_json`, `async-trait`, `tower`, `http-body-util` already exist in `third-party/BUCK` and need no add. `argon2`'s transitive crates — `password-hash`, `rand_core`, `subtle` — are generated by `buckify` and are NOT listed as standalone first-party deps: the crypto code reaches `OsRng`/`RngCore` via the `argon2::password_hash::rand_core::…` re-export, so no `//third-party:rand_core` or `//third-party:subtle` dep is ever needed.)
...
> **Implementer note (required edit, not optional):** `e2e_support.rs` currently has only an internal `use` of `EmbeddedDuckDb` — it is NOT `pub`, so `auth_e2e.rs`'s `app(...)` signature (`Arc<e2e_support::EmbeddedDuckDb>`) will not resolve as written. In Step 1, ADD `pub use query_api::serving::EmbeddedDuckDb;` (confirm the actual module path of `EmbeddedDuckDb` — grep for `struct EmbeddedDuckDb` / its existing `use` in `e2e_support.rs`) to the e2e support library's re-exports. `setup`, `StubAction`, `tref`, `land`, `grant_read`, `subject_with_role`, `session_token` (Step 1) are already `pub`. `query_api::http::{router, AppState}` are `pub`. Match whatever the existing graph/object-set e2e tests import.
...
Ingest gets the SAME authn gate (a verified subject) but no ACL gate on the materialize path (a deliberate non-goal). In `src/services/ingest/src/main.rs`, construct a `PgControlPlane` for auth. **IMPORTANT:** `pool` is moved in BOTH backend arms (the DuckLake arm passes bare `pool` to `control_plane`, the Iceberg arm passes `pool` to `IcebergMaterializer`) — so you MUST build the auth `pg` from `pool.clone()` **before** the backend match runs, and likewise capture `cfg.lock_timeout` into a local first if it is consumed inside the match. Place this block immediately after `let pool = ...` and before the `match backend`:
...
> **Implementer note:** `pool` is moved in BOTH ingest backend arms (DuckLake: `control_plane(pool, ...)`; Iceberg: `IcebergMaterializer { pool, ... }`), so the `pg` clone above MUST be created before the `match backend` block, not after it. Adjust only the wiring, not the materializer logic. The same `bootstrap_admin` env block as query-api follows the `auth_state` construction.
...
The over-the-wire e2e builds both routers and drives them with a `reqwest` client; ingest fixture tests post to `/datasets/...`. Both routes are now gated → requests without a token get `401`. **Expect a broader sweep than the query-api wire tests:** because Task 12 gates the **ingest** binary too, EVERY ingest test that drives `POST /datasets/...` through a composed/served router now 401s — this includes the ingest-side fixture/wire tests and the `spawn_http` over-the-wire harness (both backends, both services), not only the two query-api wire tests. Enumerate the full set from the grep below before editing; do not assume it is two files.
