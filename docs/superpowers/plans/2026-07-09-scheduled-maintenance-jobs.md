...
### Task 3: postgres adapter — migration 0042, atomic fire, `.sqlx`

**Files:**
- Create: `src/control-plane/postgres/migrations/0042_job_schedules.sql`
...
- [ ] **Step 1: Migration** — exactly the spec's Schema section (`queue.schedule`: `name text primary key, kind text not null, payload jsonb not null, cron text not null, next_run_at timestamptz not null`; index `schedule_due` on `next_run_at`). The migrations glob (`postgres/BUCK:127`) picks it up; `embedded_migrations_unit` asserts contiguity, not an exact count — no test edit needed.
...
- [ ] **Step 6: prek + commit** (`feat(control-plane): postgres job schedules — migration 0042, atomic fire-with-dedup`).
...
- Modify: `docs/ROADMAP.md` (REMOVE the `road-scheduled-maintenance-jobs` entry — it exists there once the spec's planning PR promoted `fut-scheduled-jobs`; also fix the write-then-derive item's `[[fut-scheduled-jobs]]` cross-link at `ROADMAP.md:14` if the planning PR has not already retargeted it)
- Modify: `docs/FUTURE.md` (`fut-iceberg-gc-orphan-sweep` links `[[fut-scheduled-jobs]]` — reword to cite the landed capability: "Pairs with scheduled GC (`/admin/schedules`, landed)"; verify no other `[[fut-scheduled-jobs]]`/`[[road-scheduled-maintenance-jobs]]` link dangles: `grep -rn "fut-scheduled-jobs\|road-scheduled-maintenance-jobs" docs/`)
