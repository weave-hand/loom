...
- Every commit: `git add` new files first, `buck2 run //tools:prek -- run --all-files` green (10 hooks incl. muntjac-check), Conventional Commit message + trailer `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>` (blank line before).
...
**Files:**
- Create: `loom_sdk/_arrow.py`; Modify: `_core.py`, `client.py`, `aclient.py`
- Test: `tests/arrow_test.py`, extend `tests/ingest_test.py` (new)
...
- [ ] **Step 1: investigate the standalone binary's config surface** — read `src/services/standalone/src/main.rs` + `lib.rs` and `service_runtime` config: how are listen addrs, `LOOM_PG_MODE=embedded`, `LOOM_DATA_PATH`, warehouse path, and the admin user provisioned when run as a BINARY (env vars? flags?). Also find the admin-bootstrap path (`grep -rn "create-admin\|bootstrap" src/services/standalone src/services/runtime src/services/*/src/main.rs`) — the e2e needs an admin credential obtainable without a direct DB pool. Record findings in the report. If NO env/CLI path can provision an admin, STOP: report BLOCKED with what exists (controller decides: add a bootstrap env to standalone in this PR, or seed via psql from the fixture's `POSTGRES_BIN_DIR`).
- [ ] **Step 2: write the test** — `e2e_smoke_test.py` (unittest, one test method to keep boot cost single): pick two free ports; spawn `STANDALONE_BIN` as a subprocess with embedded-pg env into a `tempfile.TemporaryDirectory` data path; poll `GET {query_url}/ontology/types` with the admin token until ready (bounded ~60s); then drive the **async** client end-to-end: `login` → `apply(Customer, Order)` (types created) → `land_instances([Customer(...)×2])` → `land_instances([Order(...)×2])` → re-`apply` (all unchanged) → `datasets.preview("crm","customers")` shows 2 rows → `ontology.type("Order")` shows the link → one deliberate conformance violation (an Order instance missing a required field value type-wise — e.g. build a raw `models.land("Order", [{"order_id": 1}])` missing required column) asserting `ConformanceError` with `reason == "missing_required"`. Terminate the subprocess in `addCleanup` (SIGTERM, wait, kill).
