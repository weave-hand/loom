...
- Retry taxonomy preserved verbatim: parse/unknown-type/unknown-input/ambiguous/DataFusion/infer/conformance/NoSnapshot ⇒ Abandon; wire + object-store-write ⇒ Retry with `WorkerTuning::backoff(attempts)`. gRPC `NotFound` from resolution RPCs is deterministic ⇒ Abandon (survives the wire via `cp_status` — `engine-wire/src/client.rs:27`).
...
- `GrpcQueueClient::list_files(schema, name) -> Result<Vec<FileRef>>` (client.rs:150) — return type changes in Task 3; the only existing caller is `worker/src/compact.rs:29`.
...
- Empty-table fixture: port `create_empty_table` from `transform/tests/transform_e2e.rs:188` (begin_table + create_table + commit — zero files, schema resolves).
...
- [ ] **Step 1 (red):** write `core/tests/transform_job.rs` — port the four `output_mode.rs` cases against `control_plane_core::OutputMode`, plus payload-compat pins:
...
- [ ] **Step 2:** run the new targets, verify FAIL (module absent): `buck2 test //src/control-plane/core:transform-job //src/control-plane/core:conform --unstable-allow-all-tests-on-re > /tmp/t1.log 2>&1; grep -E "Tests finished|FAIL|Error" /tmp/t1.log`
- [ ] **Step 3:** create `core/src/transform_job.rs` + `core/src/conform.rs` (conform body verbatim from transform, `use crate::{ColumnSpec, PropertyDef, UnknownLogicalType, satisfies};`); wire `lib.rs`: `mod conform; mod transform_job; pub use conform::{Violation, check_conformance}; pub use transform_job::{OutputMode, TransformJob, TypedTransformJob, TRANSFORM_JOB_KIND, TYPED_TRANSFORM_JOB_KIND};`. BUCK: mirror the `:page`-style `rust_test` blocks.
...
- Modify: `src/services/engine/src/service.rs` (`commit_transform` handler, `list_files` columns), `src/control-plane/postgres/src/iceberg_control_plane.rs:44` (ctor widening)
- Modify: `src/services/worker/src/compact.rs:29-37` (the one existing `list_files` caller)
- Create: `src/services/engine/tests/transform_wire.rs` (+ `loom_fixture_test` target in engine BUCK)
...
- [ ] **Step 2:** proto + regen (build does it — `pb-gen` genrule), engine ctor widening, engine handler + list_files columns, client `TableFiles`/`list_files`/`commit_transform`, compact.rs caller: `let live = ctx.control.list_files(...).await.map_err(...)?.files;`
...
- [ ] **Step 1 (red):** write `worker/tests/transform_e2e.rs` (mirror `compact_e2e.rs` scaffolding: `PgFixture::shared()`, shared warehouse tempdir, `local_sql_catalog` for seeding, `spawn_engine_uds(fx, &db, &wh_str, EngineOpts { control: true, flight: true, ..EngineOpts::default() })`, hand-built `Job` with `kind: TRANSFORM_JOB_KIND` and `serde_json::to_value(TransformJob{...})`). Five cases:
  1. `transform_runs_over_the_wire` — seed `main.src` via `land` with ids `[1,2,3]`; job `SELECT id FROM src WHERE id >= 2` into `main.dst`; `handle_transform` returns Ok; read `main.dst` rows back via `FlightTableClient::fetch` of its listed files (assert ids `{2,3}`); assert lineage event exists with inputs=[src] outputs=[dst] and `payload["sql"]` equal to the job SQL; assert a snapshot id exists via `IcebergCatalog::current_snapshot`.
  ...
  4. `empty_input_counts_zero` — port `create_empty_table` (transform_e2e.rs:188) against a directly-constructed `IcebergControlPlane` (test dep on postgres is fine); job `SELECT count(*) AS n FROM empty_in`; assert output table's single row is `0` (fetch + downcast).
...
- [ ] **Step 2:** implement `transform.rs` + `lib.rs` + `main.rs` + BUCK (lib deps += `//third-party:datafusion`, `//third-party:time`; `transform-e2e` target = `compact-e2e` dep list).
- [ ] **Step 3:** rerun Step 1 (PASS) + `buck2 test //src/services/worker: --unstable-allow-all-tests-on-re > /tmp/t4b.log 2>&1; grep -E "Tests finished|FAIL" /tmp/t4b.log`.
- [ ] **Step 4:** structural acceptance probe: `grep -n "sqlx\|control-plane/postgres" src/services/worker/BUCK` must hit only test-target dep lists (the `worker`/`worker-bin` blocks stay clean).
- [ ] **Step 5:** prek; commit `feat(worker): physical transform jobs over the engine wire`
...
- [ ] **Step 1 (red):** `worker/tests/typed_transform_e2e.rs` — seed backing tables via `land`, define types via `pg.ontology().define_type(ObjectType{...})` (port the `typed_transform_e2e.rs:182` pattern: name, table, properties incl. an identity-appropriate shape — copy an existing minimal type literal from that file). Three cases:
  1. `typed_transform_commits_with_type_named_lineage` — SQL in type terms; assert rows land in the output type's backing table, and the lineage event's inputs/outputs are TYPE refs (`DatasetRef::from(&TypeName)` — `loom:type` namespace) with `payload["input_tables"]`/`["output_table"]` naming the physical tables.
...
**Files:**
- Delete: `src/services/transform/` (whole directory: src, tests, BUCK, Cargo.toml)
- Modify: workspace `Cargo.toml` (drop the member), `Cargo.lock` via `cargo generate-lockfile`, `third-party/BUCK` via `./tools/buckify.sh` (only if the lock changes — transform-bin's dep set is shared, likely a no-op; commit whatever buckify regenerates)

Coverage accounting before deletion (all already ported): `output_mode`/`conform` → core (Task 1); `run_unknown_input` classification → worker e2e case 2 (Task 4); `transform_e2e` happy/empty → Task 4 cases 1/4; `overwrite_e2e` → Task 4 case 5; `typed_transform_e2e` → Task 5; `iceberg_backend_e2e`'s commit-sequence coverage → engine `transform_wire.rs` (Task 3). `transform_chain_e2e`'s absolute-path re-read is covered structurally: Task 4 case 1 already reads a transform OUTPUT (absolute paths) back over Flight, and `compact_e2e` pins absolute-path Flight reads — but add the explicit chain as case 6 in `transform_e2e.rs` if not already present: second job `SELECT * FROM dst` into `main.dst2`, assert rows (input registered from absolute-path files exercises the `scan`-equivalent wire read of an absolute live set).

- [ ] **Step 1:** add the chain case (red against nothing new — it should pass immediately; it pins the ported behavior): `buck2 test //src/services/worker:transform-e2e --unstable-allow-all-tests-on-re > /tmp/t6a.log 2>&1; grep -E "Tests finished|FAIL" /tmp/t6a.log`
- [ ] **Step 2:** `git rm -r src/services/transform`; edit workspace `Cargo.toml` members; `eval "$(./tools/env.sh)" && cargo generate-lockfile && ./tools/buckify.sh`; `grep -rn "services/transform" src/ deploy/ buildbuddy.yaml .github/ third-party/BUCK` — expect zero hits (docs handled in Task 7).
...
- Grep stale paths: `grep -rn 'services/transform' docs/` — repoint or delete every hit (ROADMAP/FUTURE prose included; `docs/system-capabilities/transform.md:7,17,55,117` are known hits).
