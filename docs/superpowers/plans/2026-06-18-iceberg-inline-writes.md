...
**Architecture:** A write primitive `inline_append` (postgres crate) stores rows in a per-table typed table `iceberg_mirror.inline_<table_id>` created on the fly, advancing a mirror snapshot and emitting lineage in one Postgres transaction. A reader `IcebergCatalog::inline_parquet` encodes a table's live inline rows to in-memory Parquet bytes (arrow/parquet 57, already in the crate). The slice-3 serving engine registers those bytes in an `InMemory` object store under `memory://` and **unions** them with the table's `file://` Parquet.

> **AS-BUILT (Task 4):** the original "add the memory:// URL to the same `ListingTable`" approach does NOT work — a single DataFusion `ListingTable` cannot span two object stores (`file://` + `memory://`); it silently scans only one and drops the other. The shipped approach builds the file Parquet and the inline Parquet as **separate** `ListingTable` providers and combines them with a DataFusion `UNION ALL` view (`ctx.read_table(file).union(ctx.read_table(inline)).into_view()`), registered under `"schema"."table"`. File-only and inline-only tables register their single provider directly. Also note: object_store 0.13's `put` is on the `ObjectStoreExt` trait (import it).
