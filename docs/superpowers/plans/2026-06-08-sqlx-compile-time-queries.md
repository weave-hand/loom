...
## Task 7: freshness `rust_test` + docs

> **Revised during execution** (the original plan proposed a prek pre-push hook;
> a skippable hook isn't a real guarantee, so the gate is a buck2 `rust_test`
> instead — it runs in the normal test sweep with the live-db fixture as an ordinary
> dependency, and buck re-runs it only when its inputs change).

**Files:** `src/control-plane/postgres/tests/sqlx_cache.rs` (new), `src/control-plane/postgres/BUCK`, `CLAUDE.md`.

- [ ] **Step 1:** Add a `rust_test` `sqlx-cache-check` that boots the fixture (migrate + DuckLake attach), reads the committed `.sqlx` via `LOOM_SQLX_DIR=$(location :sqlx-cache)/.sqlx`, and for each query re-runs `Executor::describe` against the live schema, asserting column names, exact types (`serde_json::to_value(col.type_info())` — sqlx's own `PgTypeInfo: Serialize` under the `offline` feature), nullability, and param count match. Assert the cache dir is non-empty (no vacuous pass).
- [ ] **Step 2:** Prove drift-detection: corrupt one committed `.sqlx` entry, confirm the test FAILS, restore it.
- [ ] **Step 3:** Document in `CLAUDE.md`: a "Compile-time SQL" subsection — `query!` for the five concerns (`fixture.rs` stays runtime), the offline build wiring (`SQLX_OFFLINE_DIR` via `$(location)`, `CARGO_MANIFEST_DIR="."`), `tools/sqlx-prepare.sh` as the generate/refresh tool, and the `sqlx-cache-check` rust_test as the freshness gate. Concise, in the file's voice.
- [ ] **Step 4:** Commit: `test(postgres): rust_test gating .sqlx cache freshness vs live schema`.
