# STPA Control Analysis — weave-hand/loom @ cbbf5fc
...
**Scope.** Built: control-plane library (7 concerns incl. auth), ingest service, query-api service (incl. governed Flight export + governed UPDATE/DELETE), transform service, engine service (gRPC + Flight SQL + Flight data plane + vector search), engine-serving (DataFusion/Iceberg execution + Puffin vector index), flush-worker binary, and session-based auth middleware. Designed-only: external SQL wire (Quack/Postgres-wire/Flight SQL server).

<details>
<summary>Maturity detail</summary>

- **Built:** core traits (acl with Action::Read/Write scoping, ontology with links_to + define_action + identity + ActionKind::Insert/Update/Delete, lineage, catalog, queue, auth with sessions + passwords, flush job contract, tx with enqueue/emit, vector_index with Flat/IvfFlat/Hnsw), Postgres adapter, Iceberg mirror adapter + catalog + inline writes + flush + GC + schema evolution + CAS retry, in-memory adapter, Worker loop, query-api service (handler.rs, sql.rs, serving.rs, serving_datafusion.rs, action.rs incl. run_mutate copy-on-write, write_filter.rs, filter.rs, chain_filter.rs, params.rs, path_parse.rs, http.rs, render.rs, flight_export.rs, main.rs), ingest service (materialize.rs, write.rs, bind.rs, gate.rs, landing.rs, http.rs, main.rs), transform service (handler.rs, run.rs, typed.rs, compact.rs, conform.rs, backend.rs), engine service (service.rs, flight.rs incl. VectorSearchTicket dispatch, main.rs), engine-serving (serving.rs, pg_provider.rs, vector_search.rs with cold/hot merge), engine-wire (proto + client + convert + flight), flush-worker (handler.rs, main.rs), service_runtime auth middleware (protect, require_auth, login/logout), Puffin vector index (Flat, IvfFlat, HNSW with cold Puffin + hot inline delta merge)
- **Designed-only:** External SQL wire (Quack as server for external ATTACH, Postgres-wire, external Flight SQL)
</details>
...
flowchart TD
  subgraph enforcement["Enforcement layer"]
    ...
    query-api["Query API (governed read/write: ontology resolve, ACL check + policy, link traversal with inverse hops, graph reachability, actions incl. governed UPDATE/DELETE, governed Flight export, HTTP surface, authn-gated)"]
...
| `action-engine.overwrite-table` | atomic whole-table copy-on-write rewrite with lineage (UPDATE/DELETE path) | `query-api` → `action-engine` | built | src/services/query-api/src/serving.rs:271 |
| `action-engine.write-object` | execute atomic action write: one-row Parquet + lineage via Iceberg inline append | `query-api` → `action-engine` | built | src/services/query-api/src/action.rs:299 |
...
| `engine.flush-table` | execute iceberg flush (inline rows to Parquet) for a table | `flush-worker` → `engine` | built | src/services/engine/src/service.rs:97 |
| `engine.vector-search` | k-NN vector search via Puffin cold index + inline hot delta merge | `query-api` → `serving-engine` | built | src/services/engine-serving/src/vector_search.rs:47 |
...
| `query-api.flight-export` | governed Arrow Flight columnar export (bearer-authn, ACL-governed per do_get, TCP listener) | `query-api` → `serving-engine` | built | src/services/query-api/src/flight_export.rs:190 |
...
| `serving.fetch-rows` | execute compiled read-only SQL against Iceberg catalog via DataFusion | `query-api` → `serving-engine` | built | src/services/query-api/src/serving.rs:180 |
| `tx.commit` | commit cross-concern unit of work (lineage + queue) | `ingest` → `tx` | built | src/control-plane/postgres/src/transaction.rs:37 |
| `tx.enqueue` | enqueue within transaction (visible only on commit) | `ingest` → `tx` | built | src/control-plane/postgres/src/transaction.rs:48 |
...
| `action-engine.overwrite-table.wrong-timing` | `action-engine.overwrite-table` | wrong-timing | COW reads the full table, mutates in memory, and overwrites without a CAS guard on the parent snapshot; a concurrent writer that commits between the read and the overwrite has its changes silently dropped (fut-cow-cas-guard) | high | cow-no-cas, partial-atomic-unit | src/services/query-api/src/action.rs:515 |
...
| `engine.vector-search.providing` | `engine.vector-search` | providing | vector search dispatches to the engine without ACL governance; the query-api caller's read permissions are not enforced on the returned identity/distance pairs, so a caller who can trigger a vector search sees results from rows their row-filter policy would exclude | medium | unenforced-policy | src/services/engine/src/flight.rs:61 |
...
<details>
<summary><b>Not UCAs</b> — 18 examined and rejected</summary>

- **Tx lacks catalog write (partial atomic unit)** — Tx now carries only enqueue + lineage emit; Iceberg snapshot commits via IcebergControlPlane atomically (src/control-plane/postgres/src/transaction.rs:37)
- **acl.check not issued (unbuilt query API)** — handler provides deny-by-default check before any type resolution (src/services/query-api/src/handler.rs:217)
- **action lineage non-atomic with inline write** — fixed: IcebergActionWriter routes through land, committing row + lineage in one Postgres Tx (src/services/query-api/src/action.rs:299)
...
- **define_action does not validate param conformance to target type** — now validated at invoke time by check_conformance before any insert (src/services/query-api/src/action.rs:106)
- **flight export bypasses governance** — FlightExportService re-derives ACL per do_get from bearer token via compile_object_read — same governance as HTTP reads (src/services/query-api/src/flight_export.rs:190)
...
- **multiple-role policy merging absent** — handler ANDs row_filters and unions deny/mask columns across all matching policies (src/services/query-api/src/handler.rs:229)
- **overwrite_table COW reads rows caller cannot read** — COW deliberately reads all rows unfiltered to preserve rows the caller cannot see; governance is enforced on the affected row(s) only, not the pass-through rows (src/services/query-api/src/action.rs:515)
...
- **policies fetched but never folded into query** — handler compiles row_filter trees and deny/mask columns into the SELECT via sql::compile_select (src/services/query-api/src/handler.rs:229)
...
- **write-filter UNKNOWN denies the write** — three-valued eval is fail-closed; a type mismatch or NaN denies the write rather than permitting it (src/services/query-api/src/write_filter.rs:106)
</details>
...
- The COW read-modify-overwrite window for UPDATE/DELETE has no CAS guard (fut-cow-cas-guard); a concurrent writer can commit between the read and the overwrite, silently losing its changes — is this acceptable for single-writer deployments only?
...
- Vector search results are not ACL-governed: the engine returns identity/distance pairs without checking the caller's row-filter policy, so a vector search can reveal the existence (and approximate location) of rows the caller cannot read via the governed object-read path.
