...
flowchart TD
  subgraph enforcement["Enforcement layer"]
    ...
    engine["Engine gRPC service (queue + flush delegation over UDS)"]
    flush-worker["Zero-pool flush worker binary (drains flush_table jobs via engine gRPC)"]
    ingest["Ingest service (DuckLake/Iceberg landing backends, model gate, HTTP surface)"]
...
| `unenforced-policy` | Read and action-write paths enforce policy in the query-api binary; the ingest HTTP endpoint, transform workers, and engine/flush-worker have no ACL check — any caller can land, transform, or flush data | L.unauthorized-access, L.silent-incorrectness | built |
...
<details>
<summary><b>Not UCAs</b> — 15 examined and rejected</summary>
