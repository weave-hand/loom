...
- [ ] **Stream engine — PK / CDC tables (slice 2)** `{#road-stream-pk-tables area:ontology status:planned from:2026-07-06-stream-engine-design pr:- spec:2026-07-07-stream-pk-cdc-tables-design}`
...
- [ ] **Transforms admin surface (first `SqlEditor` consumer)** `{#road-ui-transforms-surface area:ui status:planned from:2026-07-06-reusable-sql-editor-design pr:- spec:2026-07-07-transforms-admin-surface-design}`
  Build the Transforms admin surface in the Yew UI — the first real consumer of the shipped `SqlEditor` — as a list+drawer surface over the eight `/admin/transforms` routes (define/redefine, list, get, delete, run-saved, run-ad-hoc, run-history, get-run). Renames the `Pipelines` nav stub to `Transforms` (teal accent kept, `Surface::all()` stays `[Surface;5]`); reuses the existing login session's `Bearer` token and surfaces `403` as a "requires admin" empty-state (no separate admin login). The `SqlEditor`'s `CompletionSchema` is built **input-scoped** — only from each transform's declared inputs (physical: per-table `GET /datasets/{schema}/{table}`; typed: the chosen ontology types' properties) — folding in the removed `fut-ui-sql-editor-schema-wiring`. Also lands a **resizable drawer** in the shared `Shell` (the editor form is wider than existing drawers) and an epoch-guarded fetch pattern (the shipped `FetchGeneration` guard in `loom_ui_core`, `#iss-ui-async-fetch-race`). Pure parse/form/schema logic lives in `loom_ui_core` (`rust_test`'d); view glue is gallery/browser-verified. Promotes the removed `fut-ui-transforms-surface`. Out: EXPLAIN/diagnostics ([[fut-ui-sql-editor-diagnostics]]), the SQL query console ([[fut-ui-sql-query-console]]), the completion-polish proper fix ([[fut-ui-sql-completion-polish]]), any batch metadata endpoint, and live run streaming.
