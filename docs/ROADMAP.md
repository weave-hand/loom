...
- [ ] **Management API surface — CRUD the read/admin matrix gaps** `{#road-api-management-crud area:devx status:planned from:2026-07-03-api-docs-coverage-design pr:- spec:2026-07-03-api-management-crud-design}`
  Administer loom end-to-end over HTTP (promoted from `#fut-api-management-crud`): per-type ontology schema read (`GET /ontology/types/{name}`, wire-capable via the existing gov RPCs), dataset list + detail (new `Catalog::list_tables` read), define/delete routes for links and actions, grant list/revoke (`Acl::list_grants` new, `revoke` exists), role deletion (safe — every reference cascades), and user↔role assign/unassign + roles-of. Six small new trait methods across both adapters + testkit; every route documented in the #344 OpenAPI fragments (the drift guard forces it); management reads served from the direct control plane (GC-enqueue precedent, no new wire RPCs). **Out** (operator decision 2026-07-03): ontology type delete + schema-evolution update — deferred to [[fut-ontology-type-delete]] with the dangling-reference grounding (non-FK ACL grants/policies, non-cascading inbound links, the ARCHITECTURE.md migration question).
