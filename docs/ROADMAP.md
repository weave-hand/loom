...
- [ ] **Retire GitHub Actions CI (Step B)** `{#road-retire-github-ci area:devx status:planned from:buildbuddy-ci pr:- spec:2026-06-23-buildbuddy-ci-workflows-design}`
  Gated follow-up to [[road-buildbuddy-ci]]: once the BuildBuddy workflow is confirmed green on a real push and PR, delete `.github/workflows/ci.yml` and the orphaned `setup-buck2`/`install-bsdtar` composite actions, and trim the now-stale GitHub Actions prose in CLAUDE.md. Not started until live-green — removing it early leaves `main` with no CI.
