...
- [ ] **Iceberg ActionEngine (governed object writes)** `{#road-iceberg-actionengine area:iceberg status:planned from:2026-06-16-iceberg-adapter-read-path-design pr:- spec:2026-06-22-iceberg-actionengine-design}`
  First parity gap on the path to **Iceberg-default** ([[fut-replace-ducklake-decision]]). Replace the query-api `UnsupportedActionEngine` with an `IcebergActionWriter` that builds the one-row action batch and routes it through `iceberg_landing::land` (the inline-write seam), so a governed typed-insert commits the row and its lineage in one Postgres transaction and is immediately readable through the Iceberg serving engine. Insert-only, canonical scalars; no default flip in this slice. Promoted from [[fut-iceberg-actionengine]]. Inherits [[iss-iceberg-inline-visibility]]/[[iss-iceberg-inline-reparse]] semantics from the landing path.
- [ ] **Iceberg overwrite/replace write mode** `{#road-iceberg-overwrite-mode area:iceberg status:planned from:iceberg-roadmap pr:- spec:2026-06-22-iceberg-overwrite-mode-design}`
  Second parity gap on the path to **Iceberg-default** ([[fut-replace-ducklake-decision]]). Add an `overwrite_parquet_snapshot` primitive: end-cap all live `iceberg_mirror.data_file` rows at the new snapshot (leaving table/columns live) + project the new files + Iceberg append + lineage, all in one Postgres transaction — mirror-faithful replace matching DuckLake's `Tx::replace_files` contract (the Iceberg twin of `snapshot_replace.rs`). Time travel preserved; replaced files keep per-column stats. Promoted from [[fut-iceberg-overwrite]]; a dependency of the transform-output-to-Iceberg slice (planned next). External raw-metadata faithfulness deferred (same class as [[iss-iceberg-inline-visibility]], reclaimed by [[fut-iceberg-gc]]).
