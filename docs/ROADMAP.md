...
- [ ] **Python build infrastructure via muntjac + vendored uv** `{#road-python-build-infra area:build status:planned from:fut-python-bindings pr:- spec:2026-07-21-python-build-infra-muntjac-design}`
  Promoted (with [[road-python-sdk-v1]]) from `#fut-python-bindings`. loom has no Python dependency machinery — the gap that blocks the Python SDK and previously deferred the pyiceberg differential oracle. The spec commits the reindeer-pattern adoption of muntjac (weave-hand/muntjac, its first production use — dogfood findings fixed upstream, per the operator decision): a release workflow so muntjac ships prebuilt binaries, pinned `//tools:muntjac` and `//tools:uv` genrules, root `muntjac.toml` (python 3.13 matching the hermetic CPython, linux x86_64+aarch64 only), `tools/pybuckify.sh` as the buckify.sh analog, a `muntjac-check` prek hook mirroring reindeer-check, and an acceptance `python_test` importing httpx/pyarrow/pydantic from `//third-party/python:*` on RE. Sequenced before [[road-python-sdk-v1]].
...
- [ ] **Python SDK v1 (`loom-sdk`) — ingest + pydantic ontology** `{#road-python-sdk-v1 area:devx status:planned from:fut-python-bindings pr:- spec:2026-07-21-python-sdk-v1-design}`
  Promoted (with [[road-python-build-infra]]) from `#fut-python-bindings`. A hand-written Python SDK at `src/sdk/python/` (distribution `loom-sdk`, import `loom_sdk`): sync `Client` + `AsyncClient` with identical surfaces over a shared sans-IO core (httpx shells; pyarrow encodes Arrow IPC), covering the write path — `datasets.land`/`models.land`, `admin.define_model`/`define_link`, and the verification reads — with a typed error hierarchy (incl. `ConformanceError` carrying the ingest 422 violations). pydantic ships as an optional extra: `LoomModel` classes whose annotations map to ontology properties (`Identity[T]`, `Link[Other]`), an idempotent bootstrap-aware `ontology.apply` (zero-row land materializes the backing table before `define_model`), and `land_instances`. Tested by mocked-transport `python_test` units plus one e2e smoke booting ingest+engine+query-api on the hermetic fixture. Depends on [[road-python-build-infra]].
