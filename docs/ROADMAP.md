...
- [ ] **Stream engine — Merge engines (replace-class: LastRow / FirstRow / Versioned)** `{#road-stream-merge-engines area:ontology status:planned from:2026-07-08-stream-merge-engines-design pr:- spec:2026-07-08-stream-merge-engines-design}`
  Generalizes the two per-identity current-state fold sites — compaction (`consolidate_stream`) and merge-on-read (`build_merge_view`), today hardcoded to **LastRow** — into a per-CDC-table, declared merge engine. Lands the three *replace-class* engines: **LastRow** (default, byte-identical), **FirstRow** (earliest `loom_offset` wins — first-write-wins dedup), and **Versioned** (a user-declared domain **version** property sets precedence; highest version wins with `loom_offset` tie-break, so out-of-order arrival sorts correctly). A winning delete (`-D`) drops the identity, uniformly across all three. The version column is a new first-class ontology type property mirroring `identity` (`ObjectType.version` + `object_type.version` + `version_for_table`); the engine choice lives on `stream.stream_table.merge_engine`, declared at the `?merge_engine=` CDC bind and validated (`versioned` requires the type's version property; the engine is immutable post-declare). The durable changelog is engine-agnostic and untouched. Promotes `#fut-stream-merge-engines` (carved down to its replace-class engines). **Out (deferred):** the *aggregate-class* engines — Aggregation and PartialUpdate, which *combine* rows rather than pick one and need per-column ontology merge-policy + a `GROUP BY` fold path ([[fut-stream-merge-aggregate]]). Builds on the shipped slice-2b CDC dual-table + LastRow compaction; orthogonal to the read-side slices ([[road-stream-subscribe]] / [[road-stream-continuous]] / [[road-stream-joins]]).
