...
- **Freshness** is enforced by the pre-push `sqlx-prepare` hook (`tools/sqlx-prepare.sh --check` regenerates the cache and `git diff --exit-code`s it). CI's normal build is the backstop — a stale or missing cache makes the `query!` macros fail the build.
