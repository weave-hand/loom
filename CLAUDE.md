...
- **Three git dependencies, all pinned to a `main` rev rather than a release: `iceberg`, `liquid-cache`, and `vortex` (pulled in by liquid-cache).** The five iceberg-consuming manifests (`src/control-plane/postgres`, `src/services/engine`, `src/services/engine-serving`, `src/services/runtime`, `src/services/worker`) declare `iceberg = { git = "https://github.com/apache/iceberg-rust", rev = "<sha>" }`; `src/services/engine-serving` additionally declares `liquid-cache-datafusion-local` from `XiangpengHao/liquid-cache`. reindeer's non-vendored mode emits one `git_fetch` rule per *repo* (not per crate), so vortex's 24 crates cost one rule. The **whole tree is on a single arrow major (59)** with **DataFusion 55** — there are no split `arrow-*-58` target families. **Why none of these is a release:** published `iceberg` 0.10.1 is still on arrow 58, and published `liquid-cache` 0.1.13 is on DataFusion 53 / arrow 58 — both *older* than this tree, so a release pin would mean going backwards. **To bump any pin:** change the `rev`, `cargo generate-lockfile` (hermetic cargo via `eval "$(./tools/env.sh)"`), then `./tools/buckify.sh` and run the **full** `buck2 test //src/...` — an API regression shows up only in the fixture suite, not the per-crate build. **Exit** each pin when upstream publishes a release built against arrow 59 / DataFusion 55.
