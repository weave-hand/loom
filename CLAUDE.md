...
- **The `loom v1` Projects board** (org project #1, id `PVT_kwDOEV2iVs4BeJ8b`,
  ...
  Done). Labels are authoritative; a **cloud** session drives the board via
  `$LOOM_PROJECT_TOKEN` (a `project`-scoped PAT, reaching Projects GraphQL around
  the App proxy via the `api.github.com` NO_PROXY bypass), and when that token is
  unset or unreachable the helper skips softly so the next local session
  reconciles.
