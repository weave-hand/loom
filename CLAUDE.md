...
- **`//tools:prek`** — git pre-commit hook runner ([j178/prek](https://github.com/j178/prek)), a fast Rust reimplementation of pre-commit, pinned in `tools/BUCK`. Invoke as `buck2 run //tools:prek -- <args>`; hooks are configured in `prek.toml` at the repo root (run them with `… -- run --all-files`, install the git hook with `… -- install`). Ships as `.tar.gz` (not a bare `.zst` like reindeer), so the genrule untars with `tar`/`gzip` and needs no `uses_xz` label — it builds fine on RE. To bump prek: update `PREK_VERSION` in `tools/BUCK` and refresh each `sha256` from the matching `<asset>.tar.gz.sha256` sidecar on the release page.
...
GitHub Actions, at `.github/workflows/ci.yml` (repo: `rsJames-ttrpg/loom`). On push to `main` and on PRs it installs the pinned buck2 release, checks out the prelude submodule recursively, and runs `buck2 build //src/...` + `buck2 test //src/...`.
...
- **Not yet wired** (planned): a btd-driven *affected-targets* job (build/test only what a PR's diff impacts, via `//tools:supertd` + `//tools:btd`) and a lint job (`//tools:rustfmt --check` + `tools/clippy-all.sh`).
